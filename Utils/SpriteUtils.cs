...
namespace Satchel
{
    ...
    public class SpriteUtils
    {
        ...
        public static Texture2D ExtractTextureFromSpriteLegacy(Sprite testSprite,Texture2D replacetex=null, bool saveTriangles = false)
        {
            if(replacetex==null)
            {
                replacetex=testSprite.texture;
            }
            var testSpriteRect = (replacetex.width, replacetex.height);
            List<Vector2Int> texUVs = new List<Vector2Int>();
        ...
        {
            return ExtractTextureFromSpriteLegacy(testSprite,null,saveTriangles); // use legacy mode till the experimental one is fixed
        }
    }
