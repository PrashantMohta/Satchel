...
namespace Satchel
{
    ...
    public class SpriteUtils
    {
        ...
        public static Texture2D ExtractFileTextureFromSpriteLegacy(Sprite origsprite,Texture2D replaceTex, bool saveTriangles = false)
        {
            var testSpriteRect = (origsprite.texture.width, origsprite.texture.height);
            List<Vector2Int> texUVs = new List<Vector2Int>();
            List<(Vector2Int, Vector2Int, Vector2Int)> triangles = new List<(Vector2Int, Vector2Int, Vector2Int)>();
            int i;
            bool[][] contents;
            ...
            foreach (var item in origsprite.uv)
            {
                texUVs.Add(new Vector2Int(Mathf.RoundToInt(item.x * (testSpriteRect.width - 1)), Mathf.RoundToInt(item.y * (testSpriteRect.height - 1))));
            }
            for (i = 0; i < origsprite.triangles.Length; i += 3)
            {
                triangles.Add((texUVs[origsprite.triangles[i]], texUVs[origsprite.triangles[i + 1]], texUVs[origsprite.triangles[i + 2]]));
            }
            ...
            if (saveTriangles)
                saveTriangle(contents, origsprite.name, 1000000);


            //get current sprite texture
            origTex = TextureUtils.duplicateTexture(replaceTex);
            Texture2D currentSpriteTexture = new Texture2D(width, height);
            for (x = 0; x < width; x++)
            {
                for (y = 0; y < height; y++)
            ...
            var horizontal = false;
            var vertical = false;
            if (origsprite.packed)
            {
                horizontal = (origsprite.packingRotation == SpritePackingRotation.FlipHorizontal || origsprite.packingRotation == SpritePackingRotation.Rotate180);
                vertical = (origsprite.packingRotation == SpritePackingRotation.FlipVertical || origsprite.packingRotation == SpritePackingRotation.Rotate180);
            };
