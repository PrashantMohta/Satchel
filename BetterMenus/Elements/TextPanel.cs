...
namespace Satchel.BetterMenus
{
    ...
    public class TextPanel : Element
    {
        ...
        /// /// <param name="createCustomItem">The Action(GameObject) to be called on creation. The Gameobject that will be passed in is the static panel go that will be the parent of the custom item you make.</param>
        ...
        public TextPanel(string name, Action<GameObject> createCustomItem = null, float width = 1000f,int fontSize = 35, TextPanelConfig.TextFont font = TextPanelConfig.TextFont.TrajanBold, TextAnchor anchor = TextAnchor.MiddleCenter ,string Id = "__UseName") : base(Id,name)
        {
            Name = name;
        ...
        public override GameObjectRow Create(ContentArea c, MenuScreen modlistMenu, Menu Instance, bool AddToList = true)
        {
           ...
           //todo also add support for height
            ...
            CreateCustomItem?.Invoke(option.gameObject);
            if (AddToList)
            {
